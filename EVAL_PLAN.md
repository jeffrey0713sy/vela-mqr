...
## Latest Evaluation Snapshot (500-Market Run)

- Overall positive rate (`achieved_scale=True`): `345/500 = 69.0%`
- External AUC (full pipeline): `0.7244`
- External AUC (random baseline): `0.4709`
...
- Top-decile lift (full pipeline): `1.3889x`
- Top-decile lift (random baseline): `1.0417x`
- L5 precision/recall: `0/0` (current threshold-range mismatch)
...
- Add bootstrap confidence intervals for AUC and top-decile lift on external test.
- Run sensitivity checks for alternative threshold schemes (while preserving frozen split).
- Add paper-ready table and figure references directly tied to run IDs.
